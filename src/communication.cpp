...
Communication::Communication(std::string robot_name)
{
    ...
    sub_laser_ = nh_laser.subscribe<sensor_msgs::LaserScan>("/ropod/laser/scan", 1, &Communication::laserCallback, this);

    ros::NodeHandle nh_odom;
    nh_odom.setCallbackQueue(&odom_cb_queue_);
    sub_odom_ = nh_odom.subscribe<nav_msgs::Odometry>("/ropod/odom", 1, &Communication::odomCallback, this);
...
*/
    ...
    pub_base_ref_ = nh_laser.advertise<geometry_msgs::Twist>("/ropod/cmd_vel", 1);
